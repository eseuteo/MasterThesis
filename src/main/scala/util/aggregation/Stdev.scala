...
class Stdev extends AggregateFunction[DataPoint[Double], (Long, String, Double, Double, Long), (Long, String, Double)] {
  ...
  override def add(in: DataPoint[Double],
                   acc: (Long, String, Double, Double, Long)): (Long, String, Double, Double, Long) = {
    if (acc._1 == "") {
      (in.t, in.label, in.value, in.value * in.value, 1)
    } else {
      (math.max(in.t, acc._1), in.label, acc._3 + in.value, acc._4 + in.value * in.value, acc._5 + 1)
    }
  }
  ...
  override def merge(acc: (Long, String, Double, Double, Long),
                     acc1: (Long, String, Double, Double, Long)): (Long, String, Double, Double, Long) = {
    (math.max(acc._1, acc1._1), acc._2, acc._3 + acc1._3, acc._4 + acc1._4, acc._5 + acc1._5)
  }
}
