...
class Interpolation(
    timeMeasure: String = "minutes",
    windowSizeInMinutes: Long = 10,
    mode: String = "linear"
  ...
  override def process(
    ...
    val timestampOffset =
      getTimestampOffset(context.window, elements.toList, windowSizeInMinutes)
    ...
    val midDataPoint =
      elements.toList.filter(p => p.t == expectedTimestamp).headOption

    if (midDataPoint.isEmpty) {
      val previousDataPoint =
        elements.toList.filter(p => p.t < expectedTimestamp).reverse.headOption
      val nextDataPoint =
        elements.toList.filter(p => p.t > expectedTimestamp).headOption
      ...
      mode match {
        ...
        case "locf" => {
          ...
          out.collect(
            new DataPoint[Double](expectedTimestamp, key, dataPointValue)
          )
        }
        case "nocb" => {
        ...
        case "spline" => {
          val interpolatedValue: Double =
            try {
              val interpolator = new SplineInterpolator()
                .interpolate(
                  elements.map(p => p.t.toDouble).toArray,
                  elements.map(p => p.value).toArray
                )
          ...
          out.collect(
            new DataPoint[Double](expectedTimestamp, key, interpolatedValue)
          )
        }
      }
    } else {
  ...
  def getLinearInterpolation(
      prev: Option[DataPoint[Double]],
      next: Option[DataPoint[Double]],
      time: Long
  ): Double = {
    if (!prev.isEmpty && !next.isEmpty) {
      val weight: Double =
        (time - prev.head.t).toDouble / (next.head.t - prev.head.t)
