...
object StdevMinMaxExperiment {
  def getMean(
    ...
    signal
      .keyBy(t => t.label)
      .window(SlidingEventTimeWindows.of(Time.hours(8), Time.minutes(1)))
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sinkMax: StreamingFileSink[String] = StreamingFileSink
      .forRowFormat(
        new Path("Max"),
        new SimpleStringEncoder[String]("UTF-8")
      )
    ...
    val sinkMin: StreamingFileSink[String] = StreamingFileSink
      .forRowFormat(
        new Path("Min"),
        new SimpleStringEncoder[String]("UTF-8")
      )
    ...
    getMin(hrProcessedSignal, orderMA, slideMA).map(t=> t.toString).addSink(sinkMin)
    getMax(hrProcessedSignal, orderMA, slideMA).map(t=> t.toString).addSink(sinkMax)
