...
object EntropyExperiment {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sinkEntropy: StreamingFileSink[String] = StreamingFileSink
      .forRowFormat(
        new Path("Entropy"),
        new SimpleStringEncoder[String]("UTF-8")
      )
    ...
    getSampleEntropy(hrProcessedSignal, "ABPMean", 60).map(t => t.toString).addSink(sinkEntropy)
