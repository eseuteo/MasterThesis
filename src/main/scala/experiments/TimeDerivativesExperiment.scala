...
object TimeDerivativesExperiment {
  def getMean(
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sinkInterpolation: StreamingFileSink[String] = StreamingFileSink
      .forRowFormat(
        new Path("Interpolation"),
        new SimpleStringEncoder[String]("UTF-8")
      )
      .build()

    val sinkDelta: StreamingFileSink[String] = StreamingFileSink
      .forRowFormat(
        new Path("Delta"),
        new SimpleStringEncoder[String]("UTF-8")
      )
    ...
    val sinkDeltadelta: StreamingFileSink[String] = StreamingFileSink
      .forRowFormat(
        new Path("Deltadelta"),
        new SimpleStringEncoder[String]("UTF-8")
      )
    ...
    mimicDataWithTimestamps.filter(t => t.label == "ABPMean").map(t => t.toString)
    ...
    val deltas = getDelta(hrProcessedSignal, "ABPMEan")
    getDelta(deltas, "ABPMean").map(t => t.toString).addSink(sinkDeltadelta)

    deltas.map(t => t.toString).addSink(sinkDelta)
